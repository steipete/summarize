import {
  decodeBrowserAudioBytesInDocument,
  extractBrowserFfmpegFramesInDocument,
  transcodeBrowserAudioBytesInDocument,
} from "../background/browser-ffmpeg";
...
chrome.runtime.onMessage.addListener((message: BrowserFfmpegRequest, _sender, sendResponse) => {
  ...
  if (message.type === "youtube-local:transcribe") {
    ...
    void (async () => {
      ...
      const audio = await decodeBrowserAudioBytesInDocument(downloaded.bytes).catch(async () => {
        report("Preparing audio with FFmpeg WebAssembly...");
        return await transcodeBrowserAudioBytesInDocument({
  ...
  if (message.type !== "ffmpeg-wasm:frames") return;
