...
- Antigravity normal text summaries run in a temporary cwd with `--sandbox`; attachment prompts keep the caller cwd so agy can inspect the requested path but do not auto-approve tools.
