...
- Antigravity normal text summaries run `agy --print <prompt>` in a temporary cwd with `--sandbox`. Large prompts exceeding platform argv limits are automatically offloaded to a secure temporary file (`0o600`) and passed to `agy` via a `file://` reference. Attachment prompts keep the caller cwd so agy can inspect the requested path but do not auto-approve tools.
